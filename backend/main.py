...
def health_check(db: Session = Depends(get_db)):
    try:
        db.execute("SELECT 1")
        logger.info('Health check endpoint accessed, database is healthy')
...
def api_get_equipments(db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully fetched {len(equipments)} equipments")
        return equipments
    except Exception as e:
...
def api_get_equipment(equipment_id: int, db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully fetched equipment with ID: {equipment_id}")
        return db_equip
...
def api_get_parts(equipment_id: int | None = None, db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully fetched {len(parts)} parts for equipment_id: {equipment_id}")
        return parts
    except Exception as e:
...
def api_get_part(part_id: int, db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully fetched part with ID: {part_id}")
        return db_part
    except HTTPException:
        raise
    except Exception as e:
...
def api_get_workshops(db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully fetched {len(workshops)} workshops")
        return workshops
    except Exception as e:
...
def api_get_workshop(workshop_id: int, db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully fetched workshop with ID: {workshop_id}")
        return db_workshop
    except HTTPException:
        raise
    except Exception as e:
...
def api_get_replacement_types(db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully fetched {len(types)} replacement types")
        return types
    except Exception as e:
...
def api_get_replacement_type(type_id: int, db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully fetched replacement type with ID: {type_id}")
        return db_type
    except HTTPException:
        raise
    except Exception as e:
...
def api_get_replacements(equipment_id: int | None = None, db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully fetched {len(replacements)} replacements for equipment_id: {equipment_id}")
        return replacements
    except Exception as e:
...
def api_get_wear(equipment_id: int, current_date: date | None = None, db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully calculated wear for equipment_id: {equipment_id}")
        return wear
    except Exception as e:
...
def api_get_procurement(part_id: int, end_date: date, db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully calculated procurement plan for part_id: {part_id}")
        return procurement
    except Exception as e:
